...
def score_schedule(entries, course_loads=None, greedy_window_count=None):
    ...
    penalties += h3 * HARD_PENALTY
    breakdown['H3_single_pair_day'] = {'violations': h3, 'penalty': h3 * HARD_PENALTY}

    # H4: gaps (windows) between pairs within a day for a group
    h4 = _window_violations(rows, 'group_name')
    penalties += h4 * HARD_PENALTY
    breakdown['H4_group_windows'] = {'violations': h4, 'penalty': h4 * HARD_PENALTY}

    # --- Tier 2: SOFT constraints ---

    # S1: gaps between pairs within a day for a teacher (weight x2)
    s1 = _window_violations(rows, 'teacher_id')
    penalties += s1 * 2
    breakdown['S1_teacher_windows'] = {'violations': s1, 'penalty': s1 * 2}
...
def _max_achievable(course_loads, greedy_window_count):
    ...
    if greedy_window_count is not None:
        forced_penalties += greedy_window_count * HARD_PENALTY
