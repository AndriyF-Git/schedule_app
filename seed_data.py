...
# (name, difficulty)  1=легкий, 2=середній, 3=важкий
SUBJECTS = [
    ('Математичний аналіз',          3),
    ('Лінійна алгебра',              3),
    ('Фізика',                       3),
    ('Алгоритми та структури даних', 3),
    ('Бази даних',                   2),
    ('Програмування (Python)',       2),
...
def seed():
    with app.app_context():
        ...
        subjects = {name: Subject(name=name, difficulty=diff) for name, diff in SUBJECTS}
        ...
        print(f'\nDone! Seeded:')
        print(f'  {len(SUBJECTS)} subjects (difficulty: {sum(1 for _,d in SUBJECTS if d==3)} hard, {sum(1 for _,d in SUBJECTS if d==2)} medium, {sum(1 for _,d in SUBJECTS if d==1)} easy)')
        ...
        print(f'\nForced H3 violations (1-session English): 4 groups x 1 = 4')
